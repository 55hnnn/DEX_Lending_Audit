...
contract DexTest is Test {
    ...
    function test_another() external {
        uint lpToken = dex.addLiquidity(1000 ether, 1000 ether, 0);
        console.log(tokenX.balanceOf(address(dex)));
        console.log(tokenY.balanceOf(address(dex)));
        
        uint poolAmountX = 1000 ether;
        uint poolAmountY = 1000 ether;

        // x -> y
        uint firstOutputX = dex.swap(100 ether, 0, 0);
        uint firstOutputY = dex.swap(0, 100 ether, 0);
        for(uint i = 0; i<1 ; i++) {
            uint OutputX = dex.swap(1_000_000_000 ether, 0, 0);
            uint OutputY = dex.swap(0, 100 ether, 0);

            console.log(int(firstOutputX)-int(OutputX));
            console.log(int(firstOutputY)-int(OutputY));
            console.log();

            firstOutputX = OutputX;
            firstOutputY = OutputY;
        }
        ...
        uint dexLiquidity = tokenX.balanceOf(address(dex)) * tokenY.balanceOf(address(dex));
        assertEq(dex.totalLiquidity(), dex._sqrt(dexLiquidity), "is liquidity stolen?");
    }
}